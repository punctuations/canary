...
const Notification = (props: {
	...
	React.useEffect(() => {
		if (localStorage && !hidden) {
			setHiddenState(JSON.parse(localStorage.getItem('dismissed')!));
		}
	}, []);
			...
			{!hidden && (
				<motion.div
					...
					className={`relative group mt-2 text-black dark:text-white flex ${
						...
						props.variant?.toLowerCase() !== 'macOS' && props.href ? 'cursor-pointer' : ''
					...
					>
						...
						<div
							className={`flex flex-col ${
								props.variant?.toLowerCase() === 'ios' ? '-space-y-1' : ''
							}`}
						>
							<h4
								className={`font-medium ${
									props.variant?.toLowerCase() === 'macos' ? 'text-sm' : 'text-base'
								}`}
							>
								{props.title}
							</h4>
							<p className={props.variant?.toLowerCase() === 'macos' ? 'text-xs' : 'text-sm'}>
								{props.text}
							</p>
						</div>
					</div>
